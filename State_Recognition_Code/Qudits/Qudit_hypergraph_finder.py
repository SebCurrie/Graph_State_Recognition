...
def IS_RUN_State(state,d,n):
    RUN=False
    roots=[]
    w=cmath.exp(2*math.pi*1j/d)
    for i in range(d):
        roots.append(np.around(w**i,8))  
        
    
    #subroutine to check if a state is equivalent to a RUN state up to a global phase
    for root in roots:
        if root==1:
            continue
        else:
            ...
            for coeff in state:
                ...
                if  np.around(coeff/phase,8) in roots:
                    count=count+1
                    continue
                else:
                    break
            if count==d**n:
                print('This is a RUN state with a global phase:',np.around(phase,4) )  
                RUN=True             
                break
            else:
                continue
    
    RUN_state=state/phase
...
d=3
w=cmath.exp(2*math.pi*1j/d)
input_coeffs=np.array([w,w,w,w,w,w,w,w,w,w**2,w**2,w**2,w**3,w**5,w**7,w**4,w**8,w**12,w**3,w**3,w**3,w**5,w**9,w**13,w**7,w**15,w**23])

#To test if we can recognise graphs with a global phase
input_coeffs=input_coeffs*-(18.3423+9j)


#Function calls
state_strings=generate_strings(n,d,input_coeffs)
RUN, RUN_state=IS_RUN_State(input_coeffs,d,n)
qudit_hypergraph_from_RUN(RUN_state,state_strings,d,n)
